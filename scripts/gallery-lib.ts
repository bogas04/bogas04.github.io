...
export function publicSourceUrl(sourcePath: string): string {
  const relativePath = path.relative(PUBLIC_DIRECTORY, sourcePath);
  if (relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
    throw new Error(`${formatPath(sourcePath)} is not inside public`);
  }
